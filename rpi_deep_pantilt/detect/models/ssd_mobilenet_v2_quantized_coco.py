...
class SSDMobileNet_V2_Quantized_Coco(object):
    ...
    def __init__(
        ...
        self.model_dir = tf.keras.utils.get_file(
            fname=model_name,
            origin='http://download.tensorflow.org/models/object_detection/ssd_mobilenet_v3_large_coco_2019_08_14.tar.gz',# base_url + self.model_file,
            untar=True
        )

        self.model_dir = pathlib.Path(self.model_dir)
        self.model_path = str(self.model_dir) + '/model.tflite' #'/model.tflite'
